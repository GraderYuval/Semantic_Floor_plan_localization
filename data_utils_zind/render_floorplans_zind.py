...
def render_jpg_image(
    ...
    *,
                ...
                if isinstance(extruded_ring, MultiPolygon):
                    for part in extruded_ring.geoms:
                        ...
                        cv2.polylines(
                            ...
                            isClosed=True,
                            color=color,
                            thickness=12,  # border thickness is already encoded in the geometry
                            lineType=cv2.LINE_8,
                        )
                else:
    ...
    # # 5) Optional "auto-connection" lines for 'wall_only'
    # if rendering_type == "wall_only":
    #     # Flatten out points for ROOM or DOOR polygons
    #     all_points = []
    #     for polygon, scaled_points in zip(polygon_list, scaled_polygons):
    #         if polygon.type in [PolygonType.ROOM]:
    #             all_points.extend(scaled_points)

    #     # For each pair of points, if they're "close", connect them:
