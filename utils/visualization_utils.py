...
def plot_prob_dist(prob_dist, resolution: float = 0.1, save_path: str = 'temp_figs', file_name: str = 'prob_dist_map.png', occ=None, pose_pred=None, ref_pose_map=None, plot_type="combined", acc=None, acc_orn=None, acc_only=False, prob_vol_gt=None):
    ...
    if occ is not None:
        ...
        prob_dist_resized = cv2.resize(prob_dist, (prob_dist_resized.shape[1], prob_dist_resized.shape[0]), interpolation=cv2.INTER_LINEAR)
        if prob_vol_gt is not None:
    ...
    if prob_vol_gt is not None:
        ...
        fig_pred, _ = plot_single_map(
            prob_dist_resized, occ, [pose_pred, ref_pose_map], H, W, resolution, 'Prediction', arrow_colors=['blue', 'green']
        )
        ...
        fig, _ = plot_single_map(prob_dist_resized, occ, [pose_pred, ref_pose_map], H, W, resolution, 'Prediction', arrow_colors=['blue', 'green'])
